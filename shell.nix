...
in pkgs.mkShell {
  ...
  shellHook = ''
    ...
    _compute_prompt() {
      ...
      local XC_colour="$(
        if [ "''${XC}" -eq 0 ] ; then
          '${pkgs.coreutils}/bin/printf' 10 ;
        elif [ "''${XC}" -lt 128 ] ; then
          '${pkgs.coreutils}/bin/printf' 9  ;
        else
          '${pkgs.coreutils}/bin/printf' 11 ;
        fi ;
      )"
      ...
      local path="$(
        '${pkgs.coreutils}/bin/realpath' -m --relative-to="''${_ROOT_DIR}" "$('${pkgs.coreutils}/bin/pwd')"
      )/"
      ...
      local is_git="$(
        '${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" rev-parse --git-dir >/dev/null 2>&1 && \
        [[ "$('${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" rev-parse --show-toplevel)" -ef "''${_ROOT_DIR}" ]] && \
        printf true ;
      )"
      local git_status="$(
        [[ ! -z "''${is_git}" ]] && \
        '${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" status --porcelain ;
      )"
      local ref="$(
        if [ ! -z "''${is_git}" ] ; then
          '${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" rev-parse --abbrev-ref HEAD 2>/dev/null || true ;
        else
          '${pkgs.coreutils}/bin/printf' '<N/A>' ;
        fi ;
      )"
      [[ "''${ref}" = "HEAD" ]] && ref="$(
        '${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" rev-parse --short HEAD 2>/dev/null || \
        '${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" branch --show-current ;
      )"
      local ref_colour="$(
        if [ -z "''${is_git}" ] ; then
          '${pkgs.coreutils}/bin/printf' 9  ;
          ...
          '${pkgs.coreutils}/bin/printf' '%s\n' "''${git_status}" | \
            '${pkgs.gnugrep}/bin/grep' -q '^.\S' ; then
          '${pkgs.coreutils}/bin/printf' 9  ;
        else
          '${pkgs.coreutils}/bin/printf' 11 ;
        fi ;
      ...
      # Nix shell hint.
      local nix_PS1='\[\033[1;38;5;13m\]NIX+QMK\[$('${pkgs.ncurses}/bin/tput' sgr0)\]'
    ...
    setup() {
      ...
      (
        ...
        (
          '${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" init -b main
        ) || XC="$(( "''${XC}" + 0x01 ))" ;
        '${pkgs.coreutils}/bin/printf' '\n' ;
      )
      ...
      [[ -d "''${_ROOT_DIR}/QMK/" ]] && (
        ...
        (
          '${pkgs.coreutils}/bin/rm' -rf -- "''${_ROOT_DIR}/QMK/" && \
          '${pkgs.coreutils}/bin/printf' "Submodule path 'QMK': deleted\n" ;
        ) || XC="$(( "''${XC}" + 0x02 ))" ;
        '${pkgs.coreutils}/bin/printf' '\n' ;
      )

      (
        '${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" config --file .gitmodules --get-regexp path | \
        ...
        (
          '${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" submodule add --branch master -- 'https://github.com/qmk/qmk_firmware/' QMK ;
        ) || XC="$(( "''${XC}" + 0x04 ))" ;
        '${pkgs.coreutils}/bin/printf' '\n' ;
      )
      ...
      [[ "''${XC}" -ne 0 ]] || (
        ...
        (
          '${pkgs.coreutils}/bin/printf' 'Set up /etc/udev/rules.d/\n' && \
            su -c "
              '${pkgs.coreutils}/bin/mkdir' -p '/etc/udev/rules.d/' &&
              '${pkgs.coreutils}/bin/cp' -rf \"''${_ROOT_DIR}/QMK/util/udev/\"* '/etc/udev/rules.d/'
      ...
      (
        '${pkgs.coreutils}/bin/printf' 'Set up /etc/udev/rules.d/\n' && \
          su -c "
            '${pkgs.coreutils}/bin/mkdir' -p '/etc/systemd/logind.conf.d/' &&
            '${pkgs.coreutils}/bin/printf' '[Login]\nRuntimeDirectorySize=99%%\n' >'/etc/systemd/logind.conf.d/runtime_directory_size.conf'
    ...
    update() {
      ...
      (
        '${pkgs.git}/bin/git' -C "''${_ROOT_DIR}" submodule update --remote --progress ;
      ) || XC="$(( "''${XC}" + 0x01 ))" ;
      '${pkgs.coreutils}/bin/printf' '\n' ;
    ...
    help() {
      '${pkgs.coreutils}/bin/cat' <<EOF | '${pkgs.man}/bin/man' -l - 2>/dev/null
